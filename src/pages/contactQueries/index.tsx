...
import { changeContactQueryStatus, getAllContactQueries } from "../../actions/queriesAction";
import { TabWithoutIcon } from "../../components/molecules/TabWithoutIcon";
import { queriesTypeTabs } from "../../constants/tabDataConstant";
import { CHANGE_CONTACT_QUERY_STATUS_RESET, GET_ALL_CONTACT_QUERIES_RESET } from "../../constants/queriesContants";
...
const ContactQueries = () => {
    ...
    const {
        ...
        data: contactQueries } = getAllContactQueriesData;
    ...
    const {
        ...
        data: changeContactQueryResponse } = changeContactQueryData;
                        ...
                        <div className="flex items-center">
                            ...
                            <div>
                                ...
                                <div className="flex flex-row justify-center items-center gap-4">
                                    <div className="text-sm text-gray-500">{ticket.companyName}</div>
                                    <div className="font-semibold text-sm text-gray-500">{ticket.status.toUpperCase()}</div>
                                </div>
                            </div>
                        </div>
                    ...
                    {expandedId === index && (
                        <div className="px-6 pb-4 text-sm text-gray-700">
                            ...
                            <div className="space-y-1">
                                ...
                                <div>
                                    <strong>Email:</strong>{' '}
                                    <a href={`mailto:${ticket.email}`} className="text-blue-600 underline">
                                        {ticket.email}
                                    </a>
                                </div>
                                <div>
                                ...
                                {(ticket.status === "unread" || ticket.status === "read") &&
                                    <div className="flex justify-end">
                                        <button
                                            onClick={() => handleStatusChange(ticket._id, ticket.status === "unread" ? "read" : "resolved")}
                                            className="text-sm bg-blue-600 text-white px-4 py-1.5 rounded-md shadow hover:bg-blue-700 transition"
                                        >
                                            {ticket.status === "unread" ? 'Mark as Read' : 'Mark as Resolved'}
                                        </button>
                                    </div>}
                            </div>
                        </div>
