...
export function BrandCampaignScreenDetails({showIcons, brandName, campaign, campaignIds}: any) {
  return (
    <div className="group relative my-1">
      <div className={`${campaignIds?.includes(campaign?._id) ? "border border-red-500 rounded" : ""} flex p-2 gap-4 hover:bg-gray-100 hover:rounded`}>
        ...
        <div className="truncate">
          <h1 className="text-[20px] font-semibold truncate">{campaign?.name}</h1>
          <p className="text-[12px] truncate">{brandName}</p>
          <p className="text-[12px] truncate">{campaign?.campaignDuration} sec</p>
